...
type TSMFile interface {
	...
	// Remap replaces the mmap backing slice using f.  Index and Reader state are
	// not re-initialized.
	Remap(f *os.File) error
	...
	for _, file := range updated {
		...
		for _, remove := range oldFiles {
			if remove == file.Path() {
				...
				// If queries running against this file, then
				// we need to move it out of the way and let them complete.  We cannot rename
				// the file while it is open because that does not work cross-platform.  Instead,
				// we hard link the existing file to a temp file and remap the existing TSMReader
				// to this new file.  We'll then delete the original file to avoid blocking callers
				// upstream.  If the process crashes, the temp file is cleaned up at startup automatically.
				if file.InUse() {
					tempPath := file.Path() + ".tmp"
					if err := os.Link(file.Path(), tempPath); err != nil {
						return err
					}

					f, err := os.Open(tempPath)
					if err != nil {
						return fmt.Errorf("error opening file %s: %v", tempPath, err)
					}
					...
					// Replace the mmap backing slice, but don't re-load the index since
					// the files are identical.
					if err := file.Remap(f); err != nil {
						return err
					}

					// Remove the old file and tombstones.  We can use the normal TSMReader.Remove()
